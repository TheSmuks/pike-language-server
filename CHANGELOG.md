...
- **`predef::cache` was missing from the bundled Roxen index.** The generator harvested `add_constant` from `roxenloader.pike` and `roxen.pike`, but `cache` injects itself from `cache.pike`, which roxenloader merely instantiates. The forwarded members `cache_lookup` and `cache_set` were indexed while the object they hang off was not. Adds `predef.cache` and its 58 members.
