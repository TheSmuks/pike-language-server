...
- **CTRL+CLICK on a method reached through a directory `module.pmod` opened an `#include`d header at an unrelated line.** Two independent causes. A file's symbol table holds declarations *cloned* from the files it inherits and `#include`s so references can resolve to them, and a clone keeps the **other** file's line and character numbers; every query that asked "what is written at this position" matched against them, comparing two files' geometry. The same leak put code lenses on arbitrary lines and painted semantic tokens at positions the document does not have. Separately, cross-file resolution returned the URI of the file it *searched* alongside the ranges of whatever it found there, so when the match was a clone the URI named one file and the ranges another — deterministically, no coincidence needed. Symbols reached through a `module.pmod`'s own inherits resolved to nothing at all, and targets reported by the Pike runtime used its line verbatim with a hardcoded column 0, where its line is often the brace below a declaration rather than the declaration.
- **Rename produced edits that broke the file.** `this`, `this_object()` and `this_program` bind to the enclosing class so that go-to-definition on them lands there — correct, and unchanged — but they are not written occurrences of the class *name*, and rename rewrote all three. Renaming `Builder` to `Maker` in `corpus/files/class-this-object.pike` turned `return this;` into `return Maker;`; applying the server's own edits and running the result gave `Attempt to call the NULL-value`. `#include "foo.pike"` was separately offered as renameable with the quoted path as the placeholder, producing `#include newname`, which Pike rejects outright.
- **Completion lost the member list the moment a prefix was typed, and its edits deleted text.** The caret sits just past the last character typed, which is a token boundary, so while a statement is unfinished tree-sitter answers the ERROR node spanning it rather than the identifier: `s->na` degraded from a three-item member list to 373 items of global scope *without* the member being typed. Every one of those items also carried a backwards two-line edit range — the prefix search accepted an identifier from the following line — so accepting a completion deleted the line break and the start of the next line.
- **Hover described a different symbol, and pointed at the wrong text.** The stdlib reverse index is keyed on the last segment of a fully-qualified name, so answering with the first match answered in insertion order: every `Stdio.File` was documented as `Bz2.File.File`'s inherit and `Array.map` as `ADT.Relation.Binary.map`. `Hover.range` was derived from whatever each tier resolved — a declaration on another line, a declaration in another *file* (line 950 of a 103-line document), or the raw cursor column, which sliced the identifier and spilled into the following tokens.
- **Signature help: five defects.** The caret just before `)` was treated as outside the call, so help vanished exactly where the last argument is typed, or showed the enclosing call. `activeParameter` counted commas inside arguments and walked past the end of the parameter list, leaving the popup open and highlighting nothing. Variadic parameters rendered as fixed-arity. `min` and `max` produced one signature whose label was raw type-descriptor text. And the lookup ignored the receiver, so `db->query(` answered with a module-private wrapper chosen by declaration order.
- **The type and call hierarchies answered with unrelated classes.** A class was reported as its own subtype and a class's *supertype* as its subtype; `subtypes` returned at most one subclass per file, hiding five of Roxen's six direct subclasses of `String`; `supertypes` reported every same-named class in the workspace as a parent. Call hierarchy paired a declaration cloned from another file with the wrong file's URI, and collected any call expression that merely overlapped a function's line range — so a call the function is nested *inside* was reported as one it makes.
- **Semantic tokens covered punctuation and repeated spans.** `this_object()` was emitted as a single token swallowing the call parentheses and a dotted `inherit` path as one token swallowing the `.`; since semantic tokens override TextMate scopes, that repaints punctuation as part of a name. Duplicate spans were emitted to a client that advertises `overlappingTokenSupport=false`.
- **Declarations the symbol table and outline were dropping.** `if (int x = m[k])` declared nothing — the block collectors opened a scope but looked for the declaration among the condition's *children* when the grammar binds the condition field directly to it — so `x` resolved to nothing from the body and from its own name; the same in `while` and `switch`. Every `protected`, `private` and `static` declaration was missing from the document outline, and `private { ... }` blocks hid everything inside them: 861 missing symbols across 93 Roxen files. `object h = class { ... }();` leaked its members into *file* scope, where completion offered them anywhere in the file.
- **The selection-range chain ran backwards and comment folding only fired at top level.** Every `parent` was contained *by* its child, which VS Code rejects outright, so expand-selection did not work at all. Comment groups were collected on the root node only and matched `line_comment` alone, so `//!` AutoDoc blocks — where nearly all Pike documentation lives — never folded.
- **Inlay hints named the wrong function and the code lens count dropped real references.** Hints picked the callee by name alone, labelling a call with a same-named function's parameter names and contradicting go-to-definition on the same token. The lens excluded any reference at the declaration's line and column without comparing the *file*, so a genuine call that happened to land at the same position elsewhere was dropped and the lens read "0 references".
- **`Image.` offered no completions at all.** `Image` is compiled into the Pike binary — there is no `Image.pmod` under `lib/modules`, only `_Image_*` helpers — so the AutoDoc walk contributed nothing, and runtime reconciliation only asked about modules the index already named. The reconciliation list is now seeded from `indices(master()->root_module)`: 9,243 → 9,949 keys, none lost, and `predef.Image.*` has 27 entries where it had none.
- **Roxen files were reported against the stock `pike` binary.** `Roxen`, `RXML` and `Variable` exist only inside a running Roxen server, and `Roxen.pmod` does not compile under Pike 8.0 even with every module and include path supplied — so every reference became an error about the environment. One 709-line module published 96 diagnostics, 75 of them this noise; it now publishes 23, all from the project's own lint. Parse and lint diagnostics never needed the compiler, so a genuine syntax error in a Roxen file still surfaces, and a plain Pike file still gets full compiler diagnostics.
...
- **An open file could drop out of workspace symbol search, find-references, rename and the hierarchies for a moment.** Indexing a file rewires its dependents, dropping their symbol tables so the wiring can be rebuilt against the symbols now available; that rebuild is deferred and coalesced so typing stays cheap. Hover, go-to-definition and completion never see the gap, because they resolve one URI through the on-demand indexer, which rebuilds what it is asked for. The six readers that sweep every entry — workspace symbol search, cross-file references, rename, implementations, call hierarchy and type hierarchy — skip whatever is empty, so for the length of the rebuild the file did not participate at all, and a cached snapshot could not stand in for it because a snapshot holds what is on disk and never the unsaved buffer. A reference missing from a rename is a wrong answer rather than a slow one. Those readers now restore the emptied entries before sweeping, bounded by how many were invalidated and each one a file the on-demand indexer would have rebuilt the moment it was asked. Pinning the suite to a single core made `workspace/symbol` fail 5 runs in 60; it is 0 in 60 after.
  ...
  Removing the window at its source — marking a dependent stale without discarding the parse it already holds, since only its cross-file wiring is in question and `stale` already withholds the entry from `getSymbolTable` — was tried and does not work: several paths read the table directly without consulting `stale`, and keeping it made them serve unwired data, which broke include-macro completion outright. The discard stays.
...
- **A truncated diagnose dependency closure said nothing.** Each dependency handed to the Pike worker is a cache eviction it performs before recompiling, so a dependency dropped by the 64-entry cap is a module the worker keeps serving from its stale cache — the file's diagnostics can then describe a version of it that no longer exists, which is a wrong answer rather than a missing one. Truncation is now logged with the count dropped and the file, deduplicated per file so a per-keystroke diagnose cannot flood the output channel. The cap itself is unchanged: measured across Roxen 6.1's 442 Pike files, the largest real closure is 17 and 94% of the tree is at 5 or below. The cap also did not bound depth at all — it compared against the number of overlays already emitted, but they are emitted post-order, so that count stays zero all the way down a chain and a 500-deep chain walked past 64 unbounded. It is now counted on the way down.
...
- **Completion after a stdlib module offered symbols the runtime cannot index, and missed half of what it can.** `Stdio.` returned 54 items where `indices(Stdio)` has 80: 14 of them — `nb_sendfile`, `SF_WERR`, `open_files`, `register_open_file` and friends — were source-level symbols of `Stdio.pmod/module.pmod` (protected declarations, `#define` macros, an inactive `#ifdef TRACK_OPEN_FILES` block, the `inherit _Stdio` name itself) that fail to compile when accepted; meanwhile everything `inherit _Stdio;` re-exports from the C module (`Buffer`, `Stat`, `Fd`, `gethostip`, the `SEEK_*`/`PROP_*` constants) and every sibling-file class (`Readline`, `Terminfo`, `FakeFile`) was absent, because the AutoDoc harvest only ever saw one source file at a time. The index generator now reconciles each module's members against the real pike binary — at generation time only, the LSP still needs no Pike for this — adding 3,576 members and pruning 108 phantoms, every one individually confirmed non-indexable by a `master()->resolv()` probe (`indices()` alone under-reports on joined modules — it would have wrongly condemned `Crypto.None`). Modules the generation host cannot answer for — absent outright, or compiled with their C core missing so `#if constant(…)` gutted them (`GLUE` without GL, `Regexp.PCRE` without libpcre, `Java`, `Yp`, …) — are deliberately left exactly as harvested, keeping their autodoc for users whose Pike has them, and are recorded as unreconciled. For reconciled modules the bundled index is authoritative and the installed module *source* is no longer merged over it; unreconciled modules still get the source parse, which now applies Pike's actual visibility rule — protected/private declarations (variables, functions and classes alike, whether marked by a prefix modifier or a wrapping `private { … }` block), macros and the inherit name itself are not indexable from outside, which `pike -M` confirms. `String.` and `Array.` had the same phantom class (`low_fuzzymatch`, `compact_diff3`, …) and are fixed by the same change; `Stdio.`, `String.`, `Array.` and `Protocols.HTTP.` now match the runtime's indexable set exactly, modulo the pre-existing completion convention that `.` hides `__`-prefixed names (so `Stdio.` offers 72 of its 80 indices). The generator's output is key-sorted and its type strings union-canonical (the runtime prints union members in per-process order), so an unchanged regeneration is byte-identical — verified by matching md5 across consecutive runs. Costs +587 KB raw on the bundled index (+53.6 KB gzipped).
- **A nested module path completed as nothing.** While `Protocols.HTTP.` is being typed the tree holds an error node, and the lhs extraction landed on the last identifier alone — `HTTP` resolves to no module, so zero items where the runtime has 79. The full dotted chain is now reassembled for the module-completion strategies.
- **Renaming a file-scope variable named `e`, `cache` or `diff` was refused.** The rename guard derived its protected set from the *last segment* of every stdlib index key, so `predef.Math.e` blocked any file-scope `e` — but a dotted member's tail is not a bare name; `e` can only ever be written `Math.e` and shadows nothing. The set now holds only names actually reachable bare — the predef builtins and the top-level module names — shrinking from thousands of entries to 359, while `write` or `Stdio` at file scope stay guarded exactly as before.
- **The file you had open was missing from the workspace index for the whole session.** Opening a file whose `inherit`, `import` or `#include` target was not yet indexed made the dependency-closure walk index that target and then invalidate its dependents, so the wiring would re-run against the now-available symbols — which meant dropping the symbol table of the file that had just been opened. Nothing rebuilt it: the lazy rebuild only fires on a query routed through the on-demand indexer, and hover, go-to-definition and semantic tokens all take that route, so the file *looked* fine while its index entry sat `stale` with no symbol table until the user typed into it. Every consumer that reads index entries directly skipped it silently — `workspace/symbol` returned nothing at all for the open file, including symbols that exist only in the unsaved buffer; find-references, call hierarchy, type hierarchy and cache persistence walked past it the same way. A full workspace scan did not repair it either, since discovery filters on whether a file *has an entry*, not whether that entry holds anything. Open documents are now re-indexed from their buffers after each open, in dependency order — repairing a base class re-invalidates its subclasses, so bases have to go first or the subclasses are simply left stale again, which is what an editor restoring a whole inherit chain of tabs produces. Mutually inheriting files have no such order, and repairing either member empties the other; whatever a cycle leaves behind is given a symbol table without rewiring anything, which cannot invalidate further and so always terminates. Requests arriving together are coalesced into one pass, and if a repair ever does finish with an open document still empty it now says so in the log rather than returning quietly. Editing is untouched: the repair never runs per keystroke, so the stale-mark plus lazy-rebuild design that keeps typing cheap still holds.
...
- **The constants the compiler defines are now offered in completion** — `__FILE__`, `__LINE__`, `__DIR__`, `__DATE__`, `__TIME__`, `__VERSION__`, `__MAJOR__`, `__MINOR__`, `__BUILD__`, `__REAL_VERSION__`, `__PIKE__`, `__AUTO_BIGNUM__` and `UNDEFINED`. They are offered as constants rather than calls, so none of them inserts a `(`.
...
- **The introspect dependency was unpinned, and the tag pmp picked is missing the function the worker calls.** `pike.json` asked for `github.com/TheSmuks/pike-introspect` with no version, so pmp resolved the highest tag — `v1.0.0`, which is an *ancestor* of `v0.2.0` (it was tagged a day earlier, on older content) and has no `Discover.resolve_symbol`. That is precisely what `worker.pike` calls, so `resolve` failed for every symbol. `pike.lock` pins the right commit, but pmp discards the whole lockfile when any one entry is missing from the store and re-resolves from `pike.json` — which is what CI did. Now pinned to `#v0.2.0`, matching how `PUnit` was already declared, so the outcome no longer depends on the lockfile surviving.
- **CI ran the Pike-oracle suite before installing the module that oracle needs.** `pmp install` is what puts the introspect module at `modules/pike_introspect`, and the directory is gitignored — so a checkout alone never has it. `INTROSPECT_PATH` resolves from that path and the worker is spawned with `-M $INTROSPECT_PATH` only when it resolved, so with the module absent every `worker.resolve()` answered `Introspect module not available` and each caller degraded to no answer instead of failing loudly. The install steps ran *after* `bun run test`, which left the oracle half present: `Image` and `Parser.HTML` — C modules, reachable only through the runtime resolver — hovered as nothing, while the `.pmod` directory modules kept passing, so one environment gap looked like three unrelated hover bugs. The install now precedes the run, and a step asserts the module is there rather than letting the suite degrade silently.
- **Two hover tests asserted a cross-file answer without waiting for the index.** `openDoc` returns once the notification is written; indexing the file and the files it inherits happens afterwards. Asserting immediately raced that work — the same test passed in a full run, where an earlier file had already warmed the index, and failed run alone. Tests now wait on the index itself (`waitForIndexed`) rather than on a fixed sleep, so they cost only what the work costs and cannot pass by luck.

- **A bundled Roxen entry no longer prints its signature twice.** Entries arrive in two shapes: the harvested ones keep the declaration and the prose apart, while the 499 AutoDoc-derived ones — `RXML.Tag`, `RXML.Frame`, `result`, `args`, everything someone writing an RXML tag reads — carry the signature rendered into the documentation as well. Both were being emitted, so `mixed result` appeared twice; and `RXML.Tag`, whose signature is empty, opened with a blank `pike` code fence. Each entry is now rendered according to its shape.
- **A getter/setter pair is now one property, under the name readers write.** Pike spells a property as `` Config `conf() `` plus `` void `conf=(Config c) ``, and every reader writes plain `conf` — including through inheritance, where `class Sub { inherit R; … v = 5; … }` runs the setter. The server recorded both accessors under their declared spelling, so `conf` matched nothing at all: not bare inside a subclass, not as `id->conf`, and the member appeared twice in its scope. This is a Pike feature rather than a Roxen one, so it is fixed in the symbol table and applies to any codebase; the Roxen index generator had the same blind spot and now records the property too.
- **A bare member of an inherited class the workspace cannot open answered nothing.** `class RequestID2 { inherit RequestID; … return conf; }` is Roxen's own `ftp.pike`: `conf` belongs to the `RequestID` it inherits, but that class is injected as a global, so the workspace never resolves the inherit and there is no scope to search. Names visible from the cursor — the enclosing classes and the file — are now looked up in the bundled index as `Class.member`. A sibling class's inherit cannot answer.
...
- **Identifiers inside a macro's body answered nothing, anywhere.** `#define` was a flat text token — its substitution text had no structure the server could see, so every name written inside a macro body was invisible to hover, go-to-definition and references alike. 1,403 defines across the Roxen 6.1 corpus expose 4,479 such identifiers, a third of everything the audit had left to find. `#define` is now a real node with `name`, `parameters` and `body` fields, so the define collector reads the macro's name directly instead of regex-matching the directive text, and body identifiers become ordinary references. At `disk_cache.pike:653`, hovering `rmold` inside `#define DELETE_AND_RETURN(){rmold(...)}` now reads `void rmold(string fname)` and jumps to its declaration at line 611; both answered nothing before. Four macro-invocation gaps close with it, in `global_variables.pike`, `cachestatus.pike`, `proxy.pike` and `http.pike`.
...
- **A name inside `#if`, `#ifdef` or `#undef` answered nothing.** Conditional directives were one opaque token too, same as `#define` before it: `SECURITY_DEBUG` inside `#ifdef SECURITY_DEBUG` in `roxen_master.pike:120` had no position at all, so go-to-definition returned null there, and a macro used only inside an `#ifdef` looked unused to the linter even though it plainly was. Trapped identifiers across the Roxen corpus drop from 2,316 to 47, and 2,203 positions inside conditional directives become real nodes, 191 of them resolving to a same-file declaration. `defined`, `constant` and `efun` — the preprocessor's own operators, not Pike symbols — are recognised and excluded rather than reported as unresolved.
...
- **Dotted Roxen API symbols — over half the bundled index — could not be hovered at all.** `RXML.` and `Roxen.` account for 446 of the index's 719 symbols and are only ever written dotted, but every Roxen lookup went through the bare-name path: the cursor rests on `Tag`, which means nothing on its own, while the entry is keyed `RXML.Tag`. Hovering `Roxen.True` in expression position answered nothing even though the symbol was right there in the shipped index. The module-path hover tier now also tries a dotted Roxen lookup, gated on the file being a Roxen file like every other Roxen-specific tier, so a plain Pike file is still never offered them.
- **A malformed bundled Roxen index failed silently.** `asRoxenIndex` already degraded to an empty index when the shipped `roxen-index.json` did not parse, but said nothing about it — and silence there is indistinguishable from "this is not a Roxen file", which is a long thing for a user to debug. The other three static indices (stdlib, predef builtins, predef autodoc) already reported a failed load; the Roxen index now does too, logged as a warning naming the file. It matters more now that the index has grown to 313 KB and is load-bearing for every Roxen user.
- **A string-literal inherit path was painted as if it were a namespace.** `inherit "other.pike";` names its target with a string literal, and the declaration's name range is that literal, quotes included — emitting a `namespace` semantic token over it repainted a string as a namespace and fought the TextMate grammar, which colours it as the string it is. Found by the new `tools/lsp-audit/client-surface-check.mjs`, which fails on any token landing inside a comment or literal: 592 tokens checked, 1 violation. The path still resolves and still carries a document link; it simply is not a name token, and a bare-identifier inherit is unaffected.
...
- **Every LSP position on a line with a non-ASCII character before it was wrong, in both directions.** The position converter was built on the premise, stated in its own header, that tree-sitter emits UTF-8 byte offsets — but web-tree-sitter 0.26 indexes JS-string input in UTF-16 code units, exactly what LSP already requires. Every conversion therefore corrupted an already-correct value: emitted ranges shifted left by one per non-ASCII character preceding the token on its line, and incoming position lookups (hover, go-to-definition, Ctrl+click) shifted right onto a different node entirely. `positionConverter.ts` and `offsetMap.ts` — a second implementation of the same false premise — are both deleted, and the 51 call sites across 18 files that used them now pass positions through unconverted. The persisted cache format bumped to 3 so a cache built under the old, corrupting conversion cannot silently reintroduce the drift.
- **Pike source not encoded as UTF-8 was decoded as UTF-8 anyway, corrupting the text and every position derived from it.** Pike source predating universal UTF-8 is commonly ISO-8859-1 — 241 of the 442 files in the Roxen 6.1 corpus are — and every disk read hardcoded `"utf-8"`, substituting U+FFFD for each high byte and manufacturing exactly the non-ASCII characters the position drift above tripped on. `decodeSource()`/`readSource()` now resolve an explicit `#charset` directive, then valid UTF-8, then fall back to ISO-8859-1, which cannot fail — every byte sequence is valid Latin-1 — routed through the five production reads that load Pike source from disk. The `#charset` scanner matches Pike's own semantics rather than a naive regex: it ignores a directive inside a comment or string literal or a char literal (`'/*'` no longer misreads as opening a block comment), and it models Pike's backslash-newline line-splicing, so a directive spliced onto a continuation line is correctly not honoured while an ordinary code line's splice still leaves the next line eligible. `iso-2022` is aliased to `iso-2022-jp`, the label Roxen's own `japanese.pike` needs; a leading UTF-8 BOM is stripped consistently on every decode path, where it previously survived as three phantom leading characters on some of them; and a declared charset this runtime cannot honour is now surfaced rather than silently swapped for the sniffed one with no signal. Checked against the real `pike` binary (8.0.1116) as oracle on every case: 19 of 19 agree.
- **`textDocument/documentLink` never resolved against the real workspace.** `registerDocumentLinkHandler` captured the module resolver by value when the server registered its handlers, before the `initialize` handshake replaces the placeholder `WorkspaceIndex` (workspace root `/tmp/unused`, no detected Pike paths) with the real one built from `pike --show-paths`. Every link request therefore resolved against the placeholder for the life of the connection — `#include <stdio.h>` returned no link even though `stdio.h` exists in Pike's real include directory. The handler now reads the resolver through a thunk at request time, the same fix already applied to `server.ts`'s definition/hover/completion handlers for the identical bug in [0.8.49].
