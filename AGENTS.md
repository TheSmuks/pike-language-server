...
This project was generated from `ai-project-template` version **0.8.60**. See [`.template-version`](./.template-version) for the current release. Agents can read this file to determine which conventions and files to expect. Bump it (and every other version marker) with [`scripts/bump-version.sh`](./scripts/bump-version.sh) — see [Releasing](#releasing).
