...
Agents can invoke the `template-guide` skill (`.omp/skills/template-guide/SKILL.md` or Hermes skill `pike-template-guide`) to look up conventions, audit compliance, or get upgrade guidance. Agents can also invoke the Hermes skill `pike-merge-to-main` to automate the PR lifecycle after completing feature work, and the Hermes skill `pike-cut-release` to cut a new release with proper version bumping and GitHub release creation.
...
This project was generated from `ai-project-template` version **0.8.36**. See [`.template-version`](./.template-version) for the current release. Agents can read this file to determine which conventions and files to expect.
