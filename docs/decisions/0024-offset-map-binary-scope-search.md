...
Superseded in part (Q1/Q2 position conversion); M4 binary-search scope
lookup remains Accepted.

The byte→UTF-16 offset map and its `Utf16`-suffixed conversion helpers (Q1,
Q2 below) were deleted — see `docs/superpowers/plans/2026-07-29-fix-position-drift.md`,
...
this ADR's map and its conversion plumbing (Q1, Q2) were removed in Task 4.

The M4 binary-search-over-sorted-scopes decision below is unaffected and
remains live in `findScopeForNode` — only the position *conversion* it
depended on was wrong, not the search structure itself. No ADR supersedes
0024; it was retired, not replaced, so there is no reciprocal `Supersedes`
line to add elsewhere.

The rest of this document (Context/Decision/Consequences) is kept as the
historical record of the reasoning at the time for Q1/Q2; do not use it as
current guidance on how tree-sitter columns relate to LSP characters. The M4
section remains accurate current guidance.
