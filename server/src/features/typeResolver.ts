...
import type { Declaration, Reference, Scope, SymbolTable } from "./symbolTable";
...
export async function resolveMemberAccess(
  ...
  //
  // An ARRAY takes the element type even without a subscript, because `->` on
  // one automaps: `array(Obj) as; as->twice()` returns one result per element,
  // verified against Pike 8.0.1116. Without this the member fell through to
  // the bare-name fallback and answered with an unrelated same-named symbol
  // from the inherit chain.
  const automaps = declaredTypeName !== null && /^array\b/.test(declaredTypeName);
  const typeName = (indexed || automaps) && declaredTypeName
