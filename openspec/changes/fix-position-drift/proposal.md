...
- **BREAKING (internal)**: remove the UTF-8 ↔ UTF-16 conversion layer for the
  tree-sitter ⇄ LSP direction. Positions pass through unconverted. All 36 call
  sites across 15 files are deleted rather than reduced to no-op wrappers, so
...
- `server/src/util/positionConverter.ts` — reduced to the encoding-detection
  helpers that survive; the byte↔unit conversion functions are removed.
- 15 feature modules holding the 36 call sites, including `documentLink.ts`,
  `hoverHandler.ts`, `accessResolver.ts`, `signatureHelp.ts`,
  `selectionRange.ts`, and `lintRules/missingReturn.ts`.
